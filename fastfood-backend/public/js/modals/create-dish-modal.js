...
export async function open({ prefill = null } = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        function applyPrefill(formEl, dishTemplate) {
            ...
            // category might be a <select>; set only if option exists
            const category = dishTemplate.category ?? dishTemplate.type ?? '';
            const catEl = root.querySelector('#dishCategory');
            if (catEl) {
                const hasOption = [...catEl.options].some(o => o.value == category);
                catEl.value = hasOption ? category : '';
            }
