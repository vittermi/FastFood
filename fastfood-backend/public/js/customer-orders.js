...
class OrdersManager {
    ...
    async loadOrders() {
        try {
            const response = await authFetch('/api/orders/customer');
    ...
    async orderCanBeDelivered(order) {
        const deliveredStatus = 'Delivered';
        const transitions = await this.getAvailableOrderTransitions(order);

        return Array.isArray(transitions) && transitions.includes(deliveredStatus);
    }
